...
func add_cards(card_type: String):
	for c in Global.deck[card_type]:
		var new_card_scene = card_scn.instantiate()
		cards.add_child(new_card_scene)
		new_card_scene.card_set_type(card_type)
		new_card_scene.card_set_info(c)
